...
def add_file():
  files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
    ...
    initialdir=r"D:\sourceTree\Python_Project2(GUI)")  # initialdir= 최초에 사용자가 지정한 경로를 보여줌
    
  # 사용자가 선택한 파일 목록
  for file in files:    
  #   filePath = file[0:file.rindex("/")] # 마지막 선택한 파일의 폴더 위치 경로
    list_file.insert(END, file)
    #print(file)
  
# 선택삭제 함수
def del_file():
  for index in reversed(list_file.curselection()): # reversed = 뒤집기
...
def merge_image():
  ...
  dest_path = os.path.join(txt_dest_path.get(), "jh_photo.jpg")
...
# 3.파일 확장자 옵션
# 파일 확장자 옵션 라벨
lbl_format = Label(option_frame, text="확장자", width=8)
